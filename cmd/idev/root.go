...
func newRootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `idev talks to an iPhone or iPad directly over USB (or WiFi once
...
Subcommands:
  ...
  idev list                show connected devices (UDID + transport)
  idev doctor              check usbmuxd is reachable + OS prerequisites
  idev pair                trust a device (tap "Trust" when asked)
  idev info                name, iOS version, model, color, build
  idev battery             battery capacity and charging state
  idev apps                installed user apps (bundle ids)
  idev install App.ipa     install an .ipa via the Xcode zip-conduit
  idev uninstall <bundle>  remove an installed app
  idev launch <bundle>     launch an app, print its pid
  idev kill <pid>          terminate a running process
  idev syslog [--process p] [--contains s]
